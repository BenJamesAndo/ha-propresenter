...
class ProPresenterAPI:
    ...
    async def show_message(self, message_id: str, tokens: dict[str, str] | None = None) -> None:
        ...
        for token in message_data.get("tokens", []):
            ...
            if tokens and token_name in tokens:
                ...
                token_value = token_text.get("text", "") if isinstance(token_text, dict) else ""
    ...
    async def update_message(self, message_id: str, message_data: dict[str, Any]) -> None:
    ...
    async def get_audio_playlist_details(self, playlist_uuid: str) -> dict[str, Any] | None:
    ...
    async def get_media_playlist_details(self, playlist_uuid: str) -> dict[str, Any] | None:
    ...
    async def get_presentation_playlist_details(self, playlist_uuid: str) -> dict[str, Any] | None:
    ...
    async def get_media_thumbnail(self, media_uuid: str, quality: int = 400) -> bytes | None:
        ...
        try:
            ...
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    _LOGGER.warning(f"Thumbnail request failed with status {response.status}")
                    return None
        except Exception as err:
    ...
    async def presentation_seek(self, position: float) -> None:
        ...
        """
        await self._request("PUT", "/v1/transport/presentation/time", json_data=position)
    ...
    async def get_presentation_details(self, presentation_uuid: str) -> dict[str, Any] | None:
    ...
    async def trigger_slide_in_library(self, library_id: str, presentation_uuid: str, slide_index: int) -> None:
            ...
            async with async_timeout.timeout(10):
                async with session.get(f"{self.base_url}{url}") as response:
                    if response.status == 200:
                        ...
                        _LOGGER.warning("Error fetching thumbnail: %s - %s", response.status, url)
                        return None
    ...
    async def update_timer(self, timer_uuid: str, timer_config: dict) -> bool:
        ...
        """
        try:
            await self._request("PUT", f"/v1/timer/{timer_uuid}", json_data=timer_config)
            return True
        except Exception as e:
