    ...
    for timer in timers:
        ...
        if timer.get("countdown"):
            if timer_uuid and timer_name:
                entities.append(
                    ProPresenterTimerDurationNumber(coordinator, streaming_coordinator, config_entry, timer_uuid, timer_name, timer)
                )
...
class ProPresenterTimerDurationNumber(ProPresenterBaseEntity, NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        _LOGGER.debug(f"Setting timer {self._timer_name} duration to {duration_seconds}s ({value} min)")
            ...
            if current_timer_state:
                ...
                if len(parts) == 3:
                    ...
                    if pp_state == "stopped" and current_seconds == old_duration:
                        should_reset = True
                        _LOGGER.debug(f"Timer is stopped and at reset position ({current_seconds}s == {old_duration}s), will reset to apply new duration")
                    else:
                        _LOGGER.debug(f"Timer is paused or running (state={pp_state}, time={current_seconds}s != {old_duration}s), not resetting")
