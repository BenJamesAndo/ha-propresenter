...
async def async_setup_entry(
    ...
    entities.append(ProPresenterAudienceScreenSwitch(static_coordinator, streaming_coordinator, config_entry))
    entities.append(ProPresenterStageScreenSwitch(static_coordinator, streaming_coordinator, config_entry))
    
    # Add stage message switch
    entities.append(ProPresenterStageMessageSwitch(static_coordinator, streaming_coordinator, config_entry))
    
    # Add capture switch
    entities.append(ProPresenterCaptureSwitch(static_coordinator, streaming_coordinator, config_entry))
    ...
    for timer in timers:
        ...
        if timer_uuid and timer_name:
            entities.append(
                ProPresenterTimerSwitch(static_coordinator, streaming_coordinator, config_entry, timer_uuid, timer_name, timer)
            )
    ...
    for message in messages:
        ...
        if message_uuid and message_name:
            entities.append(ProPresenterMessageSwitch(static_coordinator, streaming_coordinator, config_entry, message_uuid, message_name))
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if self._capture_settings:
            ...
            if disk_settings:
                ...
                if resolution:
                    attrs["resolution"] = f"{resolution.get('width', 0)}x{resolution.get('height', 0)}"
    ...
    def icon(self) -> str:
        ...
        if self._timer_type == "countdown":
            ...
            if pp_state == "stopped":
                ...
                configured_duration = timer_config.get("countdown", {}).get("duration", 0)
